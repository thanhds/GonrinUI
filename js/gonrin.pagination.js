(function (factory) {
      ...
      */
      ...
      initialize = function () {
        ...
        var containerId = element.attr("id"),
          ...
          navNumberPageItemIdPrefix = createId(options.navNumberPageItemIdPrefix, containerId) + "_",
          ...
          if (options.showRowsPerPage) {
            html += '<div class="' + options.navListNumberOfPage + '">';
            html += '<div class="input-group">';
            html += '<p class="px-2 d-flex" style="border-right: 1px solid;"><span style="align-items: center;" class=" pt-1"> Tổng số: ' + options.totalRows + '</span></p>&nbsp;&nbsp;&nbsp;';
            html +=  `<ul id="nav_list_number_of_page" class="text-right pagination" unselectable="on" style="user-select: none;">`;
            if (options.listNumberRowsPerPage && options.listNumberRowsPerPage.length >0){
              for (let i = 0; i < options.listNumberRowsPerPage.length ; i++) { 
                let check_active = "";
                if (options.listNumberRowsPerPage[i] === options.pageSize){
                  check_active = "active";
                }
                html += `<li class="page-item px-1 `+ check_active +` "><a id="` + navNumberPageItemIdPrefix +(options.listNumberRowsPerPage[i])+`" href="javascript:void(0);" title="Change number of page" class="page-link rounded-circle d-flex justify-content-center">  style="width: 35px;height: 35px;"` + options.listNumberRowsPerPage[i] + `</a></li>`
              }
            }
            html += `</ul>`;
            html += '</div>';
        ...
        if (!options.directURL) {
          ...
          element.off("click", selectorNavPages).on("click", selectorNavPages, function (event) {
           ...
           // click change number of pages
           selector_rows_per_page = '[id^="' + navNumberPageItemIdPrefix + '"]';
           element.off("click", selector_rows_per_page).on("click", selector_rows_per_page, function (event) {
            var previousSelection = options.page;
            var len = navNumberPageItemIdPrefix.length;
            options.pageSize = parseInt($(event.target).attr("id").substr(len));
            var currentSelection = options.page;
            $("#nav_list_number_of_page .page-item").removeClass("active");
  ...
  $.fn.pagination.defaults = {
    ...
    listNumberRowsPerPage: [15, 20, 50, 100],
    showRowsInfo: false,
    showRowsDefaultInfo: true,
    containerClass: "row pagination-container borded-top-1 mx-0",
    pageClass: "page-item",
    pageOptionClass: "",
    linkClass: "page-link",
    linkOptionClass: "",
    ...
    navListClass: "col-md-6 col-12 pl-1 text-right pagination justify-content-end pt-1",
    navListNumberOfPage: "col-md-6 col-12 px-1 pt-1",
    ...
    navNumberPageItemIdPrefix: "number_page_item_",
